...
requires_statsmodels = pytest.mark.skipif(not statsmodels_available, reason="statsmodels not installed")
...
class TestAccessors:
    ...
    def test_rolling_any(self, test_window):
        ...
        assert_series_equal(
            ...
            mask["a"].rolling(test_window, min_periods=1).max().fillna(False).astype(bool_),
        )
        assert_frame_equal(
            mask.vbt.rolling_any(test_window),
            mask.rolling(test_window, min_periods=1).max().fillna(False).astype(bool_),
        )
        assert_frame_equal(
    ...
    def test_rolling_all(self, test_window):
        ...
        assert_series_equal(
            ...
            mask["a"].rolling(test_window, min_periods=1).min().fillna(False).astype(bool_),
        )
        assert_frame_equal(
            mask.vbt.rolling_all(test_window),
            mask.rolling(test_window, min_periods=1).min().fillna(False).astype(bool_),
        )
        assert_frame_equal(
