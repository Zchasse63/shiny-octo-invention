...
def generate_nb(
    ...
    out = np.full(target_shape, False, dtype=bool_)
...
def clean_enex_1d_nb(
    ...
    * `reverse_order`: If True, the order of signals is reversed.
    ...
    """
    entries_out = np.full(entries.shape, False, dtype=bool_)
    exits_out = np.full(exits.shape, False, dtype=bool_)
...
def clean_enex_nb(
    ...
    """
    entries_out = np.empty(entries.shape, dtype=bool_)
    exits_out = np.empty(exits.shape, dtype=bool_)
...
def unravel_nb(
    ...
    new_mask = np.full((mask.shape[0], n_cols), False, dtype=bool_)
...
def unravel_between_two_nb(
    ...
    new_source_mask = np.full((source_mask.shape[0], n_cols), False, dtype=bool_)
    new_target_mask = np.full((source_mask.shape[0], n_cols), False, dtype=bool_)
    source_range = np.full(n_cols, -1, dtype=int_)
    target_range = np.full(n_cols, -1, dtype=int_)
    source_idxs = np.full(n_cols, -1, dtype=int_)
    target_idxs = np.full(n_cols, -1, dtype=int_)
...
def ravel_nb(mask: tp.Array2d, group_map: tp.GroupMap) -> tp.Array2d:
    ...
    out = np.full((mask.shape[0], len(group_lens)), False, dtype=bool_)
