...
class TextSplitter(Configured):
    ...
    Args:
        first_chunk_template (Optional[CustomTemplateLike]): Template used to format the first text chunk.

            Defaults to `"$chunk_text"` (no prefix).
        chunk_template (Optional[CustomTemplateLike]): Template used to format each subsequent text chunk.
    ...
    def __init__(
        self,
        first_chunk_template: tp.Optional[tp.CustomTemplateLike] = None,
        ...
        **kwargs,
        ...
        first_chunk_template = self.resolve_setting(first_chunk_template, "first_chunk_template")
        ...
        self._first_chunk_template = first_chunk_template
        self._chunk_template = chunk_template
        self._template_context = template_context

    @property
    def first_chunk_template(self) -> tp.Kwargs:
        ...
        """
        return self._first_chunk_template
    ...
    def split_text(self, text: str) -> tp.TSTextChunks:
        ...
        for chunk_idx, (chunk_start, chunk_end) in enumerate(self.split(text)):
            ...
            chunk_template = self.first_chunk_template if chunk_idx == 0 else self.chunk_template
