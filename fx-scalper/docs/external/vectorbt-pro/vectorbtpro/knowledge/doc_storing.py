...
from vectorbtpro.knowledge.embeddings import Embedding
...
from vectorbtpro.utils.github import (
    resolve_release_name,
    resolve_asset_name,
    resolve_asset_name_from_names,
    download_github_asset,
)
from vectorbtpro.utils.parsing import get_func_arg_names
from vectorbtpro.utils.path_ import check_mkdir, remove_file, remove_dir
from vectorbtpro.utils.pickling import dumps, loads, save, load, suggest_compression, resolve_file_path
...
class StoreEmbedding(StoreObject, DefineMixin):
    ...
    embedding: tp.Optional[Embedding] = define.field(default=None)
    """Embedding instance."""
...
class ObjectStore(Configured, MutableMapping, metaclass=MetaObjectStore):
    ...
    Args:
        ...
        snapshot_dir (Optional[PathLike]): Path to the snapshot directory.
        snapshot_name (Optional[str]): Name of the snapshot file.
        snapshot_save_kwargs (KwargsLike): Keyword arguments for saving snapshots.

            See `vectorbtpro.utils.pickling.save`.
        snapshot_load_kwargs (KwargsLike): Keyword arguments for loading snapshots.

            See `vectorbtpro.utils.pickling.load`.
        auto_load_snapshot (Optional[bool]): Whether to automatically load a snapshot
            from a local file when the store is opened and found to be empty.
        auto_pull_snapshot (Optional[bool]): Whether to automatically pull a snapshot from
            a GitHub release when the store is opened and found to be empty.
        auto_save_snapshot (Optional[bool]): Whether to automatically save a snapshot
            to a local file when the store is closed.
        snapshot_pull_config (KwargsLike): Configuration for pulling snapshots from GitHub releases.
    ...
    def __init__(
        ...
        snapshot_dir: tp.Optional[tp.PathLike] = None,
        snapshot_name: tp.Optional[str] = None,
        snapshot_save_kwargs: tp.KwargsLike = None,
        snapshot_load_kwargs: tp.KwargsLike = None,
        auto_load_snapshot: tp.Optional[bool] = None,
        auto_pull_snapshot: tp.Optional[bool] = None,
        auto_save_snapshot: tp.Optional[bool] = None,
        snapshot_pull_config: tp.KwargsLike = None,
        ...
        **kwargs,
        ...
        Configured.__init__(
            ...
            snapshot_save_kwargs=snapshot_save_kwargs,
            snapshot_load_kwargs=snapshot_load_kwargs,
            auto_load_snapshot=auto_load_snapshot,
            auto_pull_snapshot=auto_pull_snapshot,
            auto_save_snapshot=auto_save_snapshot,
            snapshot_pull_config=snapshot_pull_config,
        ...
        snapshot_dir = self.resolve_setting(snapshot_dir, "snapshot_dir")
        snapshot_name = self.resolve_setting(snapshot_name, "snapshot_name")
        snapshot_save_kwargs = self.resolve_setting(snapshot_save_kwargs, "snapshot_save_kwargs", merge=True)
        snapshot_load_kwargs = self.resolve_setting(snapshot_load_kwargs, "snapshot_load_kwargs", merge=True)
        auto_load_snapshot = self.resolve_setting(auto_load_snapshot, "auto_load_snapshot")
        auto_pull_snapshot = self.resolve_setting(auto_pull_snapshot, "auto_pull_snapshot")
        auto_save_snapshot = self.resolve_setting(auto_save_snapshot, "auto_save_snapshot")
        snapshot_pull_config = self.resolve_setting(snapshot_pull_config, "snapshot_pull_config", merge=True)
        ...
        if snapshot_name is None:
            snapshot_name = f"{store_id}_snapshot"

        self._store_id = store_id
        self._purge_on_open = purge_on_open
        self._snapshot_dir = snapshot_dir
        self._snapshot_name = snapshot_name
        self._snapshot_save_kwargs = snapshot_save_kwargs
        self._snapshot_load_kwargs = snapshot_load_kwargs
        self._auto_load_snapshot = auto_load_snapshot
        self._auto_pull_snapshot = auto_pull_snapshot
        self._auto_save_snapshot = auto_save_snapshot
        self._snapshot_pull_config = snapshot_pull_config
    ...
    def snapshot_path(self) -> Path:
        ...
        Combines `snapshot_dir` and `snapshot_name`.

        Returns:
            Path: Snapshot file path.
        """
        return Path(self.snapshot_dir) / self.snapshot_name

    @property
    def snapshot_save_kwargs(self) -> tp.Kwargs:
        """Keyword arguments for saving snapshots.

        See `vectorbtpro.utils.pickling.save`.

        Returns:
            Kwargs: Dictionary of keyword arguments for saving snapshots.
        """
        return self._snapshot_save_kwargs

    @property
    def snapshot_load_kwargs(self) -> tp.Kwargs:
        """Keyword arguments for loading snapshots.

        See `vectorbtpro.utils.pickling.load`.

        Returns:
            Kwargs: Dictionary of keyword arguments for loading snapshots.
        """
        return self._snapshot_load_kwargs

    @property
    def auto_load_snapshot(self) -> bool:
        """Whether to automatically load a snapshot from a local file
        when the store is opened and found to be empty.

        Returns:
            bool: True if a snapshot will be automatically loaded; otherwise, False.
        """
        return self._auto_load_snapshot

    @property
    def auto_pull_snapshot(self) -> bool:
        """Whether to automatically pull a snapshot from
        a GitHub release when the store is opened and found to be empty.

        Returns:
            bool: True if a snapshot will be automatically pulled; otherwise, False.
        """
        return self._auto_pull_snapshot

    @property
    def auto_save_snapshot(self) -> bool:
        """Whether to automatically save a snapshot to a local file
        when the store is closed.

        Returns:
            bool: True if a snapshot will be automatically saved; otherwise, False.
        """
        return self._auto_save_snapshot

    @property
    def snapshot_pull_config(self) -> tp.Kwargs:
        """Configuration for pulling snapshots from GitHub releases.

        Returns:
            Kwargs: Dictionary of snapshot pulling parameters.
        """
        return self._snapshot_pull_config
    ...
    def on_open(self) -> None:
        """Core open logic. Subclasses override this instead of `ObjectStore.open`.
    ...
    def open(self) -> None:
        ...
        Calls `ObjectStore.on_open`, then `ObjectStore.maybe_load_snapshot`,
        then `ObjectStore.maybe_pull_snapshot`.
        ...
        self.maybe_pull_snapshot()

    def maybe_load_snapshot(self) -> None:
        """Load a snapshot from a local file if `auto_load_snapshot` is enabled
        and the store is empty.

        Returns:
            None
        """
        if self.auto_load_snapshot and len(self) == 0:
            self.load_snapshot(raise_missing=False)

    def maybe_pull_snapshot(self) -> None:
        """Pull a snapshot from a GitHub release if `auto_pull_snapshot` is enabled
        and the store is empty.

        Returns:
            None
        """
        if self.auto_pull_snapshot and len(self) == 0:
            self.pull_snapshot(raise_missing=False)

    def maybe_save_snapshot(self) -> None:
        """Save a snapshot to a local file if `auto_save_snapshot` is enabled
        and the store is not empty.

        Returns:
            None
        """
        if self.auto_save_snapshot and len(self) > 0:
            self.save_snapshot()
    ...
    def close(self) -> None:
        """Close the store by saving a snapshot (if configured), committing changes,
    ...
    def save_snapshot(self, path: tp.Optional[tp.PathLike] = None, **kwargs) -> Path:
        """Save all store contents to a pickle file for distribution.

        Args:
            path (Optional[PathLike]): File path to save to.

                If not provided, becomes `ObjectStore.snapshot_path`.
            **kwargs: Keyword arguments that override `ObjectStore.snapshot_save_kwargs`.
        ...
        kwargs = merge_dicts(self.snapshot_save_kwargs, kwargs)

        return save(dict(self.items()), path, **kwargs)

    def load_snapshot(
        self,
        path_or_bytes: tp.Union[tp.PathLike, bytes] = None,
        raise_missing: bool = True,
        **kwargs,
    ) -> None:
        """Load store contents from a pickle file or bytes.

        Args:
            path_or_bytes (Union[PathLike, bytes, None]): File path or raw bytes to load from.

                If not provided, becomes `ObjectStore.snapshot_path`.
            raise_missing (bool): Whether to raise if no matching file is found.
            **kwargs: Keyword arguments that override `ObjectStore.snapshot_load_kwargs`.
        ...
        if path_or_bytes is None:
            path_or_bytes = self.snapshot_path
        kwargs = merge_dicts(self.snapshot_load_kwargs, kwargs)

        if isinstance(path_or_bytes, bytes):
            data = loads(path_or_bytes, **kwargs)
        else:
            try:
                data = load(path_or_bytes, **kwargs)
    ...
    def pull_snapshot(self, raise_missing: bool = True, **kwargs) -> None:
        """Download a snapshot from a GitHub release and load it into this store.

        Args:
            raise_missing (bool): Whether to raise if no matching release or asset is found.
            **kwargs: Keyword arguments that override `ObjectStore.snapshot_pull_config` and
                `ObjectStore.snapshot_load_kwargs`.

        Returns:
            None
        """
        self.check_opened()
        resolve_release_names = get_func_arg_names(resolve_release_name)
        resolve_asset_names = get_func_arg_names(resolve_asset_name)
        download_names = get_func_arg_names(download_github_asset)

        pull_kwargs = {}
        load_kwargs = {}
        for k, v in kwargs.items():
            if k in resolve_release_names or k in resolve_asset_names or k in download_names:
                pull_kwargs[k] = v
            else:
                load_kwargs[k] = v

        pull_kwargs = merge_dicts(self.snapshot_pull_config, pull_kwargs)
        release_name = pull_kwargs.get("release_name", None)
        asset_name = pull_kwargs.get("asset_name", None)
        cache = pull_kwargs.get("cache", True)
        cache_dir = pull_kwargs.get("cache_dir", None)
        clear_cache = pull_kwargs.get("clear_cache", False)

        if asset_name is None:
            asset_name = self.snapshot_name

        template_context = self.template_context
        if cache and cache_dir is None:
            raise ValueError("Cache directory must be specified when cache is enabled")
        if cache and isinstance(cache_dir, CustomTemplate):
            ...
            template_context = flat_merge_dicts(dict(cache_dir=_cache_dir), template_context)
        ...
        if cache and cache_dir is not None:
            ...
            if not clear_cache and cache_dir.exists():
                new_asset_name = resolve_asset_name_from_names(
                    [file.name for file in cache_dir.iterdir() if file.is_file()],
                    asset_name=asset_name,
                    raise_missing=False,
                )
                if new_asset_name is not None:
                    cache_file = cache_dir / new_asset_name
                    if cache_file.exists():
                        return self.load_snapshot(cache_file, **load_kwargs)

        release_name_kwargs = {k: v for k, v in pull_kwargs.items() if k in resolve_release_names}
        release_name_kwargs["release_name"] = release_name
        release_name_kwargs["raise_missing"] = raise_missing
        release_name = resolve_release_name(**release_name_kwargs)
        if release_name is None:
            return None

        asset_name_kwargs = {k: v for k, v in pull_kwargs.items() if k in resolve_asset_names}
        asset_name_kwargs["release_name"] = release_name
        asset_name_kwargs["asset_name"] = asset_name
        asset_name_kwargs["raise_missing"] = raise_missing
        asset_name = resolve_asset_name(**asset_name_kwargs)
        if asset_name is None:
            return None
        ...
        warn(
            f"Asset {asset_name!r} found remotely in release {release_name!r}. "
            f"Downloading snapshot. This may take a while."
        )
        download_kwargs = {k: v for k, v in pull_kwargs.items() if k in download_names}
        download_kwargs["release_name"] = release_name
        download_kwargs["asset_name"] = asset_name
        download_kwargs["raise_missing"] = raise_missing
        result = download_github_asset(**download_kwargs)
        if "compression" not in load_kwargs:
            load_kwargs["compression"] = suggest_compression(asset_name)
        return self.load_snapshot(result, **load_kwargs)
...
class FileStore(DictStore):
    ...
    def get_next_patch_path(self) -> tp.Path:
        ...
        for file in self.store_path.glob("patch_*"):
            indices.append(self.get_patch_index(file))
        next_index = max(indices) + 1 if indices else 0
        return self.store_path / f"patch_{next_index}"

    def resolve_store_file_path(self, path: tp.PathLike) -> tp.Optional[Path]:
        """Resolve the on-disk file path for a logical store path.

        Args:
            path (PathLike): Logical path to resolve.

        Returns:
            Optional[Path]: Resolved file path if it exists, otherwise None.
        """
        compression = self.load_kwargs.get("compression", None)
        return resolve_file_path(path, compression=compression, raise_error=False)

    @staticmethod
    def get_patch_index(path: tp.PathLike) -> int:
        """Return the numeric index from a patch file name.

        Args:
            path (PathLike): Path to the patch file.

        Returns:
            int: Numeric index extracted from the patch file name.
        """
        return int(Path(path).name.split("_")[1].split(".")[0])

    def remove_store_files(self) -> None:
        """Remove both directory-backed and single-file store representations.
        ...
        store_path = self.resolve_store_file_path(self.store_path)
    ...
    def on_open(self) -> None:
        ...
        if self.store_path.exists() and self.store_path.is_dir():
            store = {}
            store.update(load(path=self.store_path / "base", **self.load_kwargs))
            patch_paths = sorted(self.store_path.glob("patch_*"), key=self.get_patch_index)
            for patch_path in patch_paths:
                store.update(load(path=patch_path, **self.load_kwargs))
            self._store = store
        else:
            store_path = self.resolve_store_file_path(self.store_path)
            if store_path is not None:
                self._store = load(path=store_path, **self.load_kwargs)
        self.reset_state()
    ...
    def commit(self) -> tp.Optional[tp.Path]:
        ...
        if self.use_patching:
            ...
            if self.consolidate:
                ...
                file_path = save(self.store, path=base_path, **self.save_kwargs)
            elif self.store_changes:
                store_file_path = self.resolve_store_file_path(self.store_path)
                if store_file_path is not None:
                    self.remove_store_files()
                base_file_path = self.resolve_store_file_path(base_path)
                if base_file_path is None:
                    file_path = save(self.store_changes, path=base_path, **self.save_kwargs)
                else:
                    file_path = save(self.store_changes, path=self.get_next_patch_path(), **self.save_kwargs)
        else:
            if self.consolidate or self.store_changes:
                if self.store_path.exists() and self.store_path.is_dir():
                    self.purge()
                file_path = save(self.store, path=self.store_path, **self.save_kwargs)
