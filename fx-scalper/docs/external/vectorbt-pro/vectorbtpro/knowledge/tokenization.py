...
if tp.TYPE_CHECKING:
    from transformers import PreTrainedTokenizerBase as HFTokenizerT
else:
    HFTokenizerT = "transformers.PreTrainedTokenizerBase"
...
class HFTokenizer(Tokenizer):
    ...
    !!! info
        For default settings, see `chat.tokenizer_configs.hf` in `vectorbtpro._settings.knowledge`.

    Args:
        model (Optional[str]): Model name or path for `transformers.AutoTokenizer.from_pretrained`.
        hf_tokenizer (Optional[PreTrainedTokenizerBase]): Pre-initialized HuggingFace tokenizer instance.
        hf_tokenizer_kwargs (KwargsLike): Keyword arguments for `transformers.AutoTokenizer.from_pretrained`.
        **kwargs: Keyword arguments for `Tokenizer` or used as `hf_tokenizer_kwargs`.
    """
    ...
    _settings_path: tp.SettingsPath = "knowledge.chat.tokenizer_configs.hf"

    def __init__(
        self,
        model: tp.Optional[str] = None,
        hf_tokenizer: tp.Optional[HFTokenizerT] = None,
        hf_tokenizer_kwargs: tp.KwargsLike = None,
        **kwargs,
    ) -> None:
        Tokenizer.__init__(
            self,
        ...
        assert_can_import("transformers")
        from transformers import AutoTokenizer

        hf_config = merge_dicts(self.get_settings(inherit=False), kwargs)
        def_model = hf_config.pop("model", None)
        def_hf_tokenizer = hf_config.pop("hf_tokenizer", None)
        def_hf_tokenizer_kwargs = hf_config.pop("hf_tokenizer_kwargs", None)

        if hf_tokenizer is None:
            hf_tokenizer = def_hf_tokenizer
        if hf_tokenizer is None:
            if model is None:
                model = def_model
            if model is None:
            ...
            for k in list(hf_config.keys()):
                if k in init_arg_names:
                    hf_config.pop(k)
            hf_tokenizer_kwargs = merge_dicts(hf_config, def_hf_tokenizer_kwargs, hf_tokenizer_kwargs)
            hf_tokenizer = AutoTokenizer.from_pretrained(model, **hf_tokenizer_kwargs)
    ...
    @property
    def hf_tokenizer(self) -> HFTokenizerT:
