...
from vectorbtpro.knowledge.embeddings import Embedding, Embeddings, resolve_embeddings
from vectorbtpro.knowledge.reranking import resolve_reranker
...
class DocumentRanker(Configured):
    ...
    Args:
        ...
        rerank (Optional[bool]): Whether to perform reranking after initial scoring.
        reranker (RerankerLike): Identifier, subclass, or instance of
            `vectorbtpro.knowledge.reranking.Reranker`.

            Resolved using `vectorbtpro.knowledge.reranking.resolve_reranker`.
        reranker_kwargs (KwargsLike): Keyword arguments to initialize or update `reranker`.
        rerank_limit (Optional[int]): Maximum number of top documents to send to the reranker.

            Only the first `rerank_limit` documents (by initial ranking score) are reranked.
        rerank_top_chunks (Union[None, bool, int]): Number of top-scoring chunks per document to send to the reranker.

            If True, only the top chunk is used. If False or None, the full document content is used.
    ...
    def __init__(
        ...
        rerank: tp.Optional[bool] = None,
        reranker: tp.RerankerLike = None,
        reranker_kwargs: tp.KwargsLike = None,
        rerank_limit: tp.Optional[int] = None,
        rerank_top_chunks: tp.Union[None, bool, int] = None,
        ...
        **kwargs,
        ...
        Configured.__init__(
            ...
            rerank_top_chunks=rerank_top_chunks,
        ...
        rerank = self.resolve_setting(rerank, "rerank")
        reranker = self.resolve_setting(reranker, "reranker", default=None)
        reranker_kwargs = self.resolve_setting(reranker_kwargs, "reranker_kwargs", default=None, merge=True)
        rerank_limit = self.resolve_setting(rerank_limit, "rerank_limit")
        rerank_top_chunks = self.resolve_setting(rerank_top_chunks, "rerank_top_chunks")
        ...
        if reranker is not None:
            reranker = resolve_reranker(reranker)
            if isinstance(reranker, type):
                reranker_kwargs = dict(reranker_kwargs)
                reranker_kwargs["template_context"] = merge_dicts(
                    template_context, reranker_kwargs.get("template_context", None)
                )
                reranker = reranker(**reranker_kwargs)
            elif reranker_kwargs:
                reranker = reranker.replace(**reranker_kwargs)
        if isinstance(rerank_top_chunks, bool):
            if rerank_top_chunks:
                rerank_top_chunks = 1
            else:
                rerank_top_chunks = None
        ...
        with ExtSettingsPath([(emb_store_cls, emb_store_settings_path)]):
            if isinstance(emb_store, type):
                ...
                if embeddings is not None:
                    snapshot_name = emb_store_kwargs.get("snapshot_name", None)
                    if snapshot_name is None:
                        snapshot_name = embeddings.snapshot_name
                    if snapshot_name is not None:
                        emb_store_kwargs["snapshot_name"] = snapshot_name
                        if emb_store_kwargs.get("auto_pull_snapshot", None) is None:
                            emb_store_kwargs["auto_pull_snapshot"] = True
        ...
        self._rerank_top_chunks = rerank_top_chunks
    ...
    @property
    def reranker(self) -> tp.Optional[tp.Reranker]:
        """Instance of `vectorbtpro.knowledge.reranking.Reranker`.
    ...
    @property
    def rerank_limit(self) -> tp.Optional[int]:
        ...
        """
        return self._rerank_limit

    @property
    def rerank_top_chunks(self) -> tp.Optional[int]:
        ...
        """
        return self._rerank_top_chunks
            ...
            for document in documents:
                ...
                if document.id_ in document_splits:
                    ...
                    if refresh_documents or refresh_embeddings or document.id_ not in self.emb_store:
                        ...
                        existing = self.emb_store[document.id_]
                        obj = existing.replace(child_ids=obj.child_ids)
                        self.emb_store[obj.id_] = obj
                else:
                    obj = self.emb_store[document.id_]
                if obj.embedding is None:
                    if obj.child_ids:
                        for child_id in obj.child_ids:
                            ...
                            if child_obj.embedding is None:
            ...
            if return_embeddings or return_documents:
                ...
                for document in documents:
                    ...
                    if obj.embedding is not None:
                        ...
                        for child_id in obj.child_ids:
                            child_obj = self.emb_store[child_id]
                            if child_obj.embedding is not None:
                                if return_documents:
                                    child_document = self.doc_store[child_id]
    ...
    def score_documents(
        ...
        with self.doc_store, self.emb_store:
            ...
            for document in documents:
                obj = self.emb_store[document.id_]
                if obj.embedding is not None:
                    obj_embeddings[obj.id_] = obj.embedding.dequantize()
                elif obj.child_ids:
                    for child_id in obj.child_ids:
                        child_obj = self.emb_store[child_id]
                        if child_obj.embedding is not None:
                            obj_embeddings[child_id] = child_obj.embedding.dequantize()
            if obj_embeddings:
                if self.embeddings is None:
                    if with_fallback:
                        raise FallbackError("Embeddings engine is not set")
                ...
                scores = self.compute_score(query_embedding.dequantize(), list(obj_embeddings.values()))
    ...
    def extract_doc_scores(
        ...
        Args:
            ...
            recursive (bool): Whether to recursively include child document scores.
        ...
        for document in scored_documents:
            ...
            if recursive and document.child_documents:
                scores.extend(cls.extract_doc_scores(document.child_documents, recursive=True))
        return scores
    ...
    def normalize_doc_scores(
        ...
        score_range: tp.Optional[tp.Tuple[float, float]] = None,
    ) -> np.ndarray:
        """Normalize a collection of scores using min-max scaling.

        Args:
            scores (Iterable[float]): Iterable of scores to normalize.
            score_range (Optional[Tuple[float, float]]): Known (min, max) bounds for the scores.

                When provided, uses these bounds for normalization instead of the
                observed min/max of the scores. When None, falls back to min-max
                normalization based on the actual score values.
    ...
    def replace_doc_scores(
        ...
        Args:
            ...
            recursive (bool): Whether to recursively replace child document scores.
        ...
        for i in range(len(scored_documents)):
            ...
            if recursive and doc.child_documents:
                child_documents = cls.replace_doc_scores(doc.child_documents, new_scores, recursive=True)
            else:
    ...
    def normalize_scored_documents(
        ...
        Args:
            ...
            score_range (Optional[Tuple[float, float]]): Known (min, max) bounds for the scores.
                ...
                When provided, uses these bounds for normalization instead of the
                observed min/max. When None, falls back to min-max normalization
                based on the actual score values.
            recursive (bool): Whether to recursively normalize child document scores.

        Returns:
            List[ScoredDocument]: Documents with normalized scores.
        """
        scores = cls.extract_doc_scores(scored_documents, recursive=recursive)
        new_scores = cls.normalize_doc_scores(scores, score_range=score_range).tolist()
        return cls.replace_doc_scores(scored_documents, new_scores, recursive=recursive)
    ...
    def sort_scored_documents(
        ...
        Args:
            scored_documents (List[ScoredDocument]): Documents with existing scores.
            reverse (bool): Whether to sort in descending order of scores.

        Returns:
            List[ScoredDocument]: Documents sorted by score.
        """
        return sorted(scored_documents, key=lambda d: (not math.isnan(d.score), d.score), reverse=reverse)
    ...
    def filter_documents(
        ...
        !!! note
            This method assumes that documents are already sorted by score in descending order.
        ...
        res_top_k = cls.resolve_top_k(scores, top_k=top_k)
        res_min_top_k = cls.resolve_top_k(scores, top_k=min_top_k)
        res_max_top_k = cls.resolve_top_k(scores, top_k=max_top_k)
        cutoff_k = cls.top_k_from_cutoff(scores, cutoff=cutoff)

        if res_top_k is not None and res_min_top_k is not None and not int_top_k and res_min_top_k > res_top_k:
            res_top_k = res_min_top_k
        if res_top_k is not None and res_max_top_k is not None and not int_top_k and res_max_top_k < res_top_k:
            res_top_k = res_max_top_k

        if cutoff_k is not None and res_min_top_k is not None and res_min_top_k > cutoff_k:
            cutoff_k = res_min_top_k
        if cutoff_k is not None and res_max_top_k is not None and res_max_top_k < cutoff_k:
            cutoff_k = res_max_top_k

        if res_top_k is None:
            res_top_k = len(scores)
        if cutoff_k is None:
            cutoff_k = len(scores)

        final_k = min(res_top_k, cutoff_k)
        if final_k == 0:
    ...
    def rerank_documents(
        ...
        """
        if self.rerank_limit is not None:
            base_documents = scored_documents[: self.rerank_limit]
        else:
            base_documents = scored_documents
        doc_texts = []
        for sd in base_documents:
            if self.rerank_top_chunks is not None and sd.child_documents:
                indexed_chunks = list(enumerate(sd.child_documents))
                top_indexed = sorted(
                    indexed_chunks,
                    key=lambda x: (not math.isnan(x[1].score), x[1].score),
                    reverse=True,
                )[: self.rerank_top_chunks]
                top_indexed.sort(key=lambda x: x[0])
                parts = []
                prev_idx = None
                for idx, chunk in top_indexed:
                    c = chunk.document.get_content()
                    if c is not None:
                        if parts and prev_idx is not None and idx != prev_idx + 1:
                            parts.append("\n\n...\n\n")
                        parts.append(c)
                        prev_idx = idx
                content = "".join(parts)
            else:
                content = sd.document.get_content()
                content = content if content is not None else ""
            doc_texts.append(content)
        reranked = self.reranker.rerank(query, doc_texts)
        reranked_documents = []
        for orig_idx, score in reranked:
            base = base_documents[orig_idx]
            reranked_documents.append(base.replace(score=score))
    ...
    def rank_documents(
        ...
        When reranking is enabled, `top_k`, `cutoff`, `min_top_k`, and `max_top_k` are applied
        to the reranked scores rather than the initial scores. Only `rerank_limit` controls
        how many documents are sent to the reranker.
        ...
        scored_documents = self.sort_scored_documents(scored_documents)
        if self.reranker is not None:
            scored_documents = self.rerank_documents(query, scored_documents)
        if self.normalize_scores:
            scored_documents = self.normalize_scored_documents(
                scored_documents,
                score_range=self.reranker.score_range if self.reranker is not None else None,
                recursive=self.reranker is None,
            )
        scored_documents = self.filter_documents(
            scored_documents,
            top_k=top_k,
            min_top_k=min_top_k,
            max_top_k=max_top_k,
