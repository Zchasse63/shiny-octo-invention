...
class VBTAsset(KnowledgeAsset):
    ...
    def pull(
        ...
        **kwargs,
        ...
        Args:
            ...
            asset_name (Optional[str]): Name of the asset file (e.g. 'messages.json').
            ...
            if assets_dir.exists():
                if clear_cache:
                    ...
                    new_asset_name = resolve_asset_name_from_names(
                        [file.name for file in assets_dir.iterdir() if file.is_file()],
                        asset_name=asset_name,
                        release_name=release_name,
                        raise_missing=False,
                    )
                    if new_asset_name is not None:
                        cache_file = assets_dir / new_asset_name
        ...
        if cache and assets_dir.exists() and not clear_cache:
        ...
        warn(
            ...
            f"Downloading knowledge asset. This may take a while."
        )
        result = download_github_asset(
            repo_owner=repo_owner,
            repo_name=repo_name,
            release_name=release_name,
            asset_name=asset_name,
            ...
            cache_dir=assets_dir if cache else None,
            cache_mkdir_kwargs=cache_mkdir_kwargs,
            show_progress=show_progress,
            pbar_kwargs=pbar_kwargs,
        )
        if isinstance(result, Path):
            return cls.from_json_file(result, release_name=release_name, **kwargs)
        else:
            compression = suggest_compression(asset_name)
            if compression is not None and "compression" not in kwargs:
                kwargs["compression"] = compression
            return cls.from_json_bytes(result, release_name=release_name, **kwargs)
    ...
    def fit_to_context(
        ...
        """Return a string representation of the asset that fits within the specified token limit.

        Behavior:

        * Return the full rendered value if it fits.
        * For non-lists, otherwise try a partial render for strings or dicts with a
        string `content` field, then fall back to a minimal same-shape value.
        * For lists, return the largest prefix of complete leading items that fits,
        optionally followed by one partial next item if that also fits.
        * If no complete list item fits, return a partial first item when possible;
        otherwise return the empty list representation.
        ...
        Returns:
            str: String representation of the asset within the token limit.
        """
        from vectorbtpro.knowledge.tokenization import tokenize, detokenize
        from vectorbtpro.utils.formatting import dump

        dump_engine = self.resolve_setting(dump_engine, "dump_engine")
        dump_kwargs = self.resolve_setting(dump_kwargs, "dump_kwargs", merge=True)
        ...
        def _render(results):
            return dump(results, dump_engine=dump_engine, **dump_kwargs)

        def _render_and_len(results):
            rendered = _render(results)
            return rendered, _tok_len(rendered)

        def _minimal_same_shape_value(results):
            ...
            if isinstance(results, str):
                return ""
            if results is None or isinstance(results, (bool, int, float)):
                return results
            raise TypeError(f"Unsupported value type: {type(results)!r}")

        def _can_partialize_value(results):
            return isinstance(results, str) or (isinstance(results, dict) and isinstance(results.get("content"), str))

        def _get_partializable_content(results):
            if isinstance(results, str):
                return results
            return results["content"]

        def _with_replaced_content(results, content):
            if isinstance(results, str):
                return content
            new_results = dict(results)
            new_results["content"] = content
            return new_results

        def _best_partial_render(results, wrap_candidate):
            if not _can_partialize_value(results):
                return None

            original_content = _get_partializable_content(results)
            content_tokens = tokenize(original_content)

            if len(content_tokens) == 0:
                return None

            lo, hi = 1, len(content_tokens)
            best_rendered = None

            while lo <= hi:
                mid = (lo + hi) // 2
                truncated_content = detokenize(content_tokens[:mid]) + TRUNC_SUFFIX
                candidate_results = _with_replaced_content(results, truncated_content)
                rendered, rendered_len = _render_and_len(wrap_candidate(candidate_results))
        ...
        if not isinstance(results, list):
            full_rendered, full_len = _render_and_len(results)
            if full_len <= max_tokens:
                return full_rendered

            partial_rendered = _best_partial_render(results, lambda candidate: candidate)
            if partial_rendered is not None:
                return partial_rendered

            fallback = _minimal_same_shape_value(results)
            fallback_rendered, fallback_len = _render_and_len(fallback)
            if fallback_len <= max_tokens:
                return fallback_rendered

            raise ValueError("No representation fits within max_tokens")

        empty_rendered, empty_len = _render_and_len([])
        if empty_len > max_tokens:
            raise ValueError("No representation fits within max_tokens")
        ...
        render_cache = {0: empty_rendered}
        render_len_cache = {0: empty_len}

        def _render_list_prefix(prefix_len):
            if prefix_len not in render_cache:
                rendered, rendered_len = _render_and_len(results[:prefix_len])
                render_cache[prefix_len] = rendered
                render_len_cache[prefix_len] = rendered_len
            return render_cache[prefix_len]

        def _render_list_prefix_len(prefix_len):
            if prefix_len not in render_len_cache:
                _render_list_prefix(prefix_len)
            return render_len_cache[prefix_len]

        if _render_list_prefix_len(1) > max_tokens:
            partial_first = _best_partial_render(results[0], lambda candidate: [candidate])
            return partial_first if partial_first is not None else empty_rendered
        ...
        probe = 2
        while probe <= n:
            if _render_list_prefix_len(probe) <= max_tokens:
                best_prefix = probe
                fit = probe
                if probe == n:
                    break
        ...
        if fail is None and best_prefix < n:
            fail = n

        if fail is not None and fit + 1 <= fail - 1:
            left, right = fit + 1, fail - 1
            while left <= right:
                mid = (left + right) // 2
                if _render_list_prefix_len(mid) <= max_tokens:
                    best_prefix = mid
                    left = mid + 1
                else:
                    right = mid - 1

        best_rendered = _render_list_prefix(best_prefix)

        if best_prefix < n:
            base_items = results[:best_prefix]
            partial_rendered = _best_partial_render(
                results[best_prefix],
                lambda candidate: base_items + [candidate],
            )
            if partial_rendered is not None:
                return partial_rendered
    ...
    def to_context(
        ...
        **kwargs,
        ...
        """Convert the asset to a context string with optional pagination, metadata
        stripping, serialization engine selection, and token truncation.

        Args:
            *args: Positional arguments for `vectorbtpro.knowledge.base_assets.KnowledgeAsset.to_context`.
            ...
            dump_engine (Optional[str]): Name of the dump engine.
                ...
                In addition, the special option "text" can be used to render each item separately
                with "nestedtext" engine and concatenate the results in a human-friendly way.
            dump_kwargs (KwargsLike): Keyword arguments for dumping structured data.
            ...
            n (Optional[int]): Number of results to return per page.
                ...
                If None, returns all results that fit within the `max_tokens` limit.
            page (int): Page number to return (1-indexed).

                Use to paginate results. For example, if `n=5` and `page=2`, it will return the 6th to 10th results.
            separator (Optional[str]): Separator used for joining dumped data in the base class path.
            **kwargs: Keyword arguments for `vectorbtpro.knowledge.base_assets.KnowledgeAsset.to_context`.

        Returns:
            str: Serialized results string, possibly truncated to fit within *max_tokens*.
        """
        ...
        if dump_engine == "text":
            dump_iter = results.dump(return_iterator=True, wrap=False, dump_engine="nestedtext", **dump_kwargs)

            TRUNC_SUFFIX = "\n\n... Truncated to fit max_tokens limit."
            info_token_count = len(tokenize(TRUNC_SUFFIX)) if max_tokens is not None else 0
            budget = (max_tokens - info_token_count) if max_tokens is not None else None
            ...
            for i, dumped in enumerate(dump_iter):
                header = f"==== Result {i + 1} ====\n\n" if multi else ""
                sep = "\n\n" if i > 0 else ""
                chunk = sep + header + dumped
                ...
                if budget is not None:
                    ...
                    if total_tokens + chunk_tokens > budget:
                        if i == 0:
                            ...
                            chunk = detokenize(tokenize(chunk)[:remaining])
                            parts.append(chunk)
                        truncated = True
                        break
            ...
            context = "".join(parts)
            if truncated:
                context += TRUNC_SUFFIX
            return context
        elif max_tokens is not None:
            return results.fit_to_context(max_tokens, dump_engine=dump_engine, dump_kwargs=dump_kwargs)
        else:
            return KnowledgeAsset.to_context(
                results,
                *args,
                separator=separator,
                dump_engine=dump_engine,
                **merge_dicts(dump_kwargs, kwargs),
            )
...
class PagesAsset(VBTAsset):
    ...
    def find_page(
        self: PagesAssetT,
        link: tp.MaybeList[str],
        aggregate: tp.Union[bool, int] = False,
        aggregate_kwargs: tp.KwargsLike = None,
        ...
        **kwargs,
        ...
        Args:
            link (MaybeList[str]): Link or list of links to search for.
            aggregate (Union[bool, int]): Whether to aggregate headings into pages.

                If an integer, specifies the maximum heading level to aggregate (e.g., 2 keeps
                H1 separate but merges H2+). True aggregates all headings, False disables.
            aggregate_kwargs (KwargsLike): Keyword arguments for `PagesAsset.aggregate`.
        ...
        if aggregate:
            ...
            if isinstance(aggregate, int) and not isinstance(aggregate, bool):
                aggregate_kwargs.setdefault("max_level", aggregate)
    ...
    def find_obj_api(
        ...
        *,
        ...
        aggregate: tp.Union[None, bool, int] = None,
        aggregate_ancestors: tp.Union[None, bool, int] = None,
        aggregate_refs: tp.Union[None, bool, int] = None,
        aggregate_kwargs: tp.KwargsLike = None,
        ...
        Args:
            ...
            aggregate (Union[None, bool, int]): Aggregate descendant headings into pages for the object
                and its base classes or attributes.
            ...
            aggregate_ancestors (Union[None, bool, int]): Override aggregation for ancestor headings.
            aggregate_refs (Union[None, bool, int]): Override aggregation for reference headings.
            aggregate_kwargs (KwargsLike): Keyword arguments for `PagesAsset.aggregate`.
    ...
    def find_obj_docs(
        ...
        **kwargs,
        ...
        Args:
            ...
            aggregate (Union[None, bool, int]): Whether to perform aggregation on matched items.

                If an integer, specifies the maximum heading level to aggregate (e.g., 2 keeps
                H1 separate but merges H2+). True aggregates all headings.
            aggregate_kwargs (KwargsLike): Keyword arguments for `PagesAsset.aggregate`.
    ...
    def aggregate(
        self: PagesAssetT,
        max_level: tp.Optional[int] = None,
        ...
        Args:
            max_level (Optional[int]): Maximum heading level to aggregate.
            
                Headings at this level and deeper are merged into their parent. For example,
                `max_level=2` keeps H1 headings as separate entries but merges H2+ into their parent.
            ...
            for child in list(children):
                if child in link_map:
                    ...
                    if child_node["type"].startswith("heading"):
                        child_level = int(child_node["type"].split(" ")[1])
                        if max_level is None or child_level >= max_level:
    ...
    def generate_llmstxt_api(
        self,
        aggregate: tp.Union[bool, int] = True,
        aggregate_kwargs: tp.KwargsLike = None,
        **kwargs,
    ) -> str:
...
def find_api(
    ...
    **kwargs,
    ...
    Args:
        ...
        aggregate (Union[None, bool, int]): Whether to aggregate headings into pages.

            If an integer, specifies the maximum heading level to aggregate (e.g., 2 keeps
            H1 separate but merges H2+). True aggregates all headings. If None, defaults to False.
        aggregate_kwargs (KwargsLike): Keyword arguments for `PagesAsset.aggregate`.
    ...
    if cache_documents:
        ...
        if asset is None:
            if not silence_warnings:
                warn("Caching documents. This may take a while.")
...
def quick_chat(
    ...
    **kwargs,
    ...
    """Process a query and generate a chat response using BM25 ranking and quick mode.
    ...
    **kwargs,
    ...
    Args:
        ...
        tools (Optional[Tools]): Tools to be used in the conversation.
            ...
            "mcp" to use the MCP tools from `vectorbtpro.mcp.tool_registry`,
