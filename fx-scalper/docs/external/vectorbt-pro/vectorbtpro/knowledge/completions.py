...
class Completions(ThoughtProcessor):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def _merge_mcp_into_registry():
            ...
            from vectorbtpro.mcp import tool_registry as mcp_tool_registry
    ...
    def get_completion_content(self, message: str) -> str:
        ...
        _ = new_completions.get_completion(message)
        return buf.getvalue()
...
def resolve_completions(completions: tp.CompletionsLike = None) -> tp.MaybeType[Completions]:
    ...
    Args:
        completions (CompletionsLike): Identifier, subclass, or instance of `Completions`.
            ...
            * "openai" for `OpenAICompletions` (remote)
            * "anthropic" for `AnthropicCompletions` (remote)
            * "gemini" for `GeminiCompletions` (remote)
            * "hf_inference" for `HFInferenceCompletions` (remote)
            * "litellm" for `LiteLLMCompletions` (remote)
            * "ollama" for `OllamaCompletions` (local)
            * "llama_index" for `LlamaIndexCompletions` (remote or local)
    ...
    if isinstance(completions, str):
        ...
        completions = resolve_provider(
            completions,
            remote_candidates=(
                ("openai", lambda: check_installed("openai") and os.getenv("OPENAI_API_KEY")),
                ("anthropic", lambda: check_installed("anthropic") and os.getenv("ANTHROPIC_API_KEY")),
