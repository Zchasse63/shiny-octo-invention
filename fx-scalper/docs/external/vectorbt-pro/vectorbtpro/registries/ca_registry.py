...
_SETUP_UPDATE_COUNTER = 0
"""Global counter that increments each time a setup is updated."""

_SETUP_UPDATE_COUNTER_LOCK = threading.Lock()
"""Lock for synchronizing access to the global setup update counter."""
...
class CABaseSetup(CAMetrics, DefineMixin):
    ...
    def __attrs_post_init__(self) -> None:
        ...
        object.__setattr__(self, "_use_cache_lut_seq", None)
        object.__setattr__(self, "_whitelist_lut_seq", None)

    @staticmethod
    def next_update_seq() -> int:
        """Return the next monotonic sequence number for setup updates.

        Returns:
            int: Sequence number that is strictly increasing within the current process.
        """
        global _SETUP_UPDATE_COUNTER

        with _SETUP_UPDATE_COUNTER_LOCK:
            _SETUP_UPDATE_COUNTER += 1
            return _SETUP_UPDATE_COUNTER

    def mark_use_cache_updated(self) -> None:
        """Record that the `use_cache` flag has been updated.

        Stores both a UTC timestamp and a monotonic sequence number so precedence can be
        resolved deterministically even when multiple updates share the same timestamp.

        Returns:
            None
        """
        object.__setattr__(self, "_use_cache_lut", datetime.now(timezone.utc))
        object.__setattr__(self, "_use_cache_lut_seq", self.next_update_seq())

    def mark_whitelist_updated(self) -> None:
        """Record that the `whitelist` flag has been updated.
        ...
        object.__setattr__(self, "_whitelist_lut_seq", self.next_update_seq())
    ...
    @property
    def use_cache_lut_seq(self) -> tp.Optional[int]:
        """Last update sequence for the `use_cache` flag.

        Returns:
            Optional[int]: Monotonic sequence number of the most recent `use_cache` update,
                or None if the flag has not been updated.
        """
        return object.__getattribute__(self, "_use_cache_lut_seq")
    ...
    @property
    def whitelist_lut_seq(self) -> tp.Optional[int]:
        """Last update sequence for the `whitelist` flag.

        Returns:
            Optional[int]: Monotonic sequence number of the most recent `whitelist` update,
                or None if the flag has not been updated.
        """
        return object.__getattribute__(self, "_whitelist_lut_seq")
...
class CAClassSetup(CABaseDelegatorSetup, DefineMixin):
    ...
    * `CAInstanceSetup` for instance setups.
    ...
    def any_use_cache_lut(self) -> tp.Optional[datetime]:
        ...
        max_use_cache_lut_seq = self.use_cache_lut_seq
        for setup in self.superclass_setups:
            if setup.use_cache_lut_seq is not None:
                if max_use_cache_lut_seq is None or setup.use_cache_lut_seq > max_use_cache_lut_seq:
                    max_use_cache_lut = setup.use_cache_lut
                    max_use_cache_lut_seq = setup.use_cache_lut_seq
        return max_use_cache_lut

    @property
    def any_use_cache_lut_seq(self) -> tp.Optional[int]:
        """Most recent `use_cache` update sequence in this class hierarchy.

        Returns:
            Optional[int]: Latest sequence number among this class setup and its
                superclass setups, or None if no update has been recorded.
        """
        max_use_cache_lut_seq = self.use_cache_lut_seq
        for setup in self.superclass_setups:
            if setup.use_cache_lut_seq is not None:
    ...
    def any_whitelist_lut(self) -> tp.Optional[datetime]:
        ...
        max_whitelist_lut_seq = self.whitelist_lut_seq
        for setup in self.superclass_setups:
            if setup.whitelist_lut_seq is not None:
                if max_whitelist_lut_seq is None or setup.whitelist_lut_seq > max_whitelist_lut_seq:
                    max_whitelist_lut = setup.whitelist_lut
                    max_whitelist_lut_seq = setup.whitelist_lut_seq
        return max_whitelist_lut

    @property
    def any_whitelist_lut_seq(self) -> tp.Optional[int]:
        """Most recent `whitelist` update sequence in this class hierarchy.
        ...
        if self.use_cache is None or self.whitelist is None:
            ...
            if self.use_cache is None:
                if (
                    ...
                    if unbound_setup.use_cache_lut_seq is not None and (
                        instance_setup.class_setup.any_use_cache_lut_seq is None
                        or unbound_setup.use_cache_lut_seq > instance_setup.class_setup.any_use_cache_lut_seq
                    ):
            ...
            if self.whitelist is None:
                if (
                    ...
                    if unbound_setup.whitelist_lut_seq is not None and (
                        instance_setup.class_setup.any_whitelist_lut_seq is None
                        or unbound_setup.whitelist_lut_seq > instance_setup.class_setup.any_whitelist_lut_seq
                    ):
