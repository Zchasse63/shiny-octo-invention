...
shortcut_config = ReadonlyConfig(
    {
        ...
        "multiplier": dict(obj_type="red_array", group_by_aware=False),
...
class Portfolio(Analyzable, SimRangeMixin, metaclass=MetaPortfolio):
    ...
    Args:
        ...
        init_price (ArrayLike): Initial position price.
            ...
            Provided in a format that supports flexible indexing.
        multiplier (ArrayLike): Contract multiplier.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        multiplier = to_1d_array(multiplier)
        if cash_deposits is None:
            cash_deposits = 0.0
    ...
    def row_stack(
        ...
        **kwargs,
        ...
        if "multiplier" not in kwargs:
            multiplier = None
            for i, obj in enumerate(_objs):
                multiplier_obj = obj.get_multiplier()
                multiplier_obj = to_1d_array(multiplier_obj)
                multiplier_obj = broadcast_array_to(multiplier_obj, n_cols)
                if multiplier is None:
                    multiplier = multiplier_obj
                elif not np.array_equal(multiplier, multiplier_obj):
                    raise ValueError("Objects to be merged must have the same multiplier")
    ...
    def column_stack(
        ...
        * One-dimensional arrays are concatenated via `vectorbtpro.base.wrapping.ArrayWrapper.concat_arrs`.
        ...
        if "multiplier" not in kwargs:
            stack_multiplier_objs = False
            for obj in _objs:
                if (to_1d_array(obj.multiplier) != 1).any():
                    stack_multiplier_objs = True
                    break
            if stack_multiplier_objs:
                kwargs["multiplier"] = to_1d_array(
                    kwargs["wrapper"].concat_arrs(
                        *[obj.multiplier for obj in _objs],
                        group_by=False,
                    ),
                )
            else:
                kwargs["multiplier"] = np.array([1.0])
        if "cash_deposits" not in kwargs:
            stack_cash_deposits_objs = False
            for obj in _objs:
                if obj._cash_deposits.size > 1 or obj._cash_deposits.item() != 0:
    ...
    def from_orders(
        ...
        multiplier: tp.Optional[tp.ArrayLike] = None,
        cash_deposits: tp.Optional[tp.ArrayLike] = None,
        cash_earnings: tp.Optional[tp.ArrayLike] = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            init_price (Optional[ArrayLike]): Initial position price.
                ...
                Broadcasts to match the final number of columns.
            multiplier (Optional[ArrayLike]): Contract multiplier.
        ...
        **kwargs,
        ...
        Args:
            ...
            multiplier (Optional[ArrayLike]): Contract multiplier.
                ...
                Defaults to `Portfolio.multiplier` with `keep_flex=True` or 1 if not provided.
            entry_trades_cls (Optional[type]): Class used for wrapping entry trade records.
        ...
        if not isinstance(cls_or_self, type):
            ...
            if multiplier is None:
                multiplier = cls_or_self.resolve_shortcut_attr(
                    "multiplier",
                    wrapper=wrapper,
                    keep_flex=True,
                )
    ...
    def get_positions(
        ...
        **kwargs,
        ...
        return positions_cls.from_trades(trades, multiplier=multiplier, **kwargs)
    ...
    def get_cash_flow(
        ...
        Args:
            ...
            multiplier (Optional[ArrayLike]): Contract multiplier.
                ...
                Defaults to `Portfolio.get_multiplier` with `keep_flex=True` or 1 if not provided.
            cash_earnings (Optional[ArrayLike]): Cash earnings or losses at the end of each bar.
    ...
    def get_multiplier(
        cls_or_self,
        multiplier_raw: tp.Optional[tp.ArrayLike] = None,
        wrapper: tp.Optional[ArrayWrapper] = None,
        wrap_kwargs: tp.KwargsLike = None,
        keep_flex: bool = False,
    ) -> tp.Union[tp.ArrayLike, tp.MaybeSeries]:
        """Get contract multiplier per column.

        Args:
            multiplier_raw (Optional[ArrayLike]): Raw contract multiplier.

                Defaults to `Portfolio._multiplier` if not provided.
            wrapper (Optional[ArrayWrapper]): Array wrapper instance.
        ...
        Returns:
            Union[ArrayLike, MaybeSeries]: Contract multiplier, possibly wrapped.
        """
        if not isinstance(cls_or_self, type):
            if multiplier_raw is None:
                multiplier_raw = cls_or_self._multiplier
            if wrapper is None:
                wrapper = cls_or_self.wrapper
        else:
            checks.assert_not_none(multiplier_raw, arg_name="multiplier_raw")
            checks.assert_not_none(wrapper, arg_name="wrapper")

        if keep_flex:
            return multiplier_raw
        multiplier = broadcast_array_to(multiplier_raw, wrapper.shape_2d[1])
        if keep_flex:
            return multiplier
        wrap_kwargs = merge_dicts(dict(name_or_index="multiplier"), wrap_kwargs)
        return wrapper.wrap_reduced(multiplier, group_by=False, **wrap_kwargs)
