                ...
                if not np.isnan(size):
                    if size_type == SizeType.Percent:
                        ...
                        if size_type == SizeType.Value:
                            order_size -= size / (val_price_now * multiplier)
                        else:
                            order_size -= size
...
@register_chunkable(
    ...
    arg_take_spec=dict(
        ...
        multiplier=base_ch.flex_array_1d_gl_slicer,
        ...
        for i in range(_sim_start, _sim_end):
            ...
            for col in range(from_col, to_col):
                if last_position[col] != 0:
                    ...
                    group_value += last_position[col] * last_val_price[col] * _multiplier
                ...
                for ci in range(group_len):
                    ...
                    if _i >= 0:
                        ...
                        _size, _size_type, _direction = signal_to_size_nb(
                            ...
                            accumulate=_accumulate,
                            multiplier=flex_select_1d_pc_nb(multiplier_, col),
                        )
        ...
        for i in range(_sim_start, _sim_end):
            ...
            if not skip:
                ...
                for col in range(from_col, to_col):
                    ...
                    order_info["fees"][col] = flex_select_nb(fees_, _i, col)
                    order_info["fixed_fees"][col] = flex_select_nb(fixed_fees_, _i, col)
                    order_info["slippage"][col] = flex_select_nb(slippage_, _i, col)
...
def dir_to_ls_signals_nb(
    ...
    Returns:
        Tuple[Array2d, Array2d, Array2d, Array2d]: Tuple containing:
            ...
            * short exits.
    """
    long_entries_out = np.empty(target_shape, dtype=bool_)
    long_exits_out = np.empty(target_shape, dtype=bool_)
    short_entries_out = np.empty(target_shape, dtype=bool_)
    short_exits_out = np.empty(target_shape, dtype=bool_)
...
def order_signal_func_nb(  # % line.replace("order_signal_func_nb", "signal_func_nb")
    ...
    order_size_type = flex_select_nb(size_type, _i, c.col)
    order_direction = flex_select_nb(direction, _i, c.col)
    ...
    if (
        ...
        size_type[_i, c.col] = SizeType.Amount
