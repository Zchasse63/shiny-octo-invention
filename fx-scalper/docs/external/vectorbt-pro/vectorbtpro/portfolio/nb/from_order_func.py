...
def calc_ctx_group_value_nb(seg_ctx: SegmentContext) -> float:
    ...
    return calc_group_value_nb(
        from_col=seg_ctx.from_col,
        to_col=seg_ctx.to_col,
        cash_now=seg_ctx.last_cash[seg_ctx.group],
        last_position=seg_ctx.last_position,
        last_val_price=seg_ctx.last_val_price,
        multiplier=seg_ctx.multiplier,
    )
...
def from_order_func_nb(  # %? line.replace("from_order_func_nb", new_func_name)
    ...
    Args:
        ...
        init_price (FlexArray1dLike): See `vectorbtpro.portfolio.enums.SimulationContext.init_price`.
            ...
            Provided as a scalar or per column.
        multiplier (FlexArray1dLike): See `vectorbtpro.portfolio.enums.SimulationContext.multiplier`.
                ...
                if cash_sharing:
                    ...
                    for col in range(from_col, to_col):
                        if last_position[col] == 0:
                            ...
                            last_value[col] = last_cash[col] + last_position[col] * last_val_price[col] * _multiplier
