...
@register_jitted
def get_col_multiplier_nb(c: tp.AnySimContext, col: int) -> float:
    """Return the multiplier for a specified column.
    ...
    Returns:
        float: Multiplier of the specified column.
    """
    return flex_select_1d_pc_nb(c.multiplier, col)


@register_jitted
def get_multiplier_nb(c: tp.AnyColumnContext) -> float:
    """Return the multiplier for the current column.

    Args:
        c (AnyColumnContext): Relevant context.

    Returns:
        float: Multiplier of the current column.
    """
    return get_col_multiplier_nb(c, c.col)
...
def get_col_position_value_nb(c: tp.AnySimContext, col: int) -> float:
    ...
    Returns:
        float: Product of the column's position, valuation price, and multiplier, or 0.0 if the position is zero.
    """
    position = get_col_position_nb(c, col)
    val_price = get_col_val_price_nb(c, col)
    multiplier = get_col_multiplier_nb(c, col)
    if position == 0:
        return 0.0
    return position * val_price * multiplier
...
def get_col_position_records_nb(
    ...
    position_records = pf_records_nb.get_positions_nb(
        exit_trade_records,
        col_map,
        multiplier=get_col_multiplier_nb(c, col),
    )
...
@register_jitted
def get_col_asset_spec_nb(c: tp.AnySimContext, col: int) -> AssetSpec:
    """Retrieve the asset specification for the specified column.
    ...
    Returns:
        AssetSpec: Asset specification for the specified column.
    """
    return AssetSpec(
        multiplier=get_col_multiplier_nb(c, col),
    )
...
@register_jitted
def get_asset_spec_nb(c: tp.AnyColumnContext) -> AssetSpec:
    """Retrieve the asset specification for the current column.

    Args:
        c (AnyColumnContext): Relevant context.

    Returns:
        AssetSpec: Asset specification for the current column.
    """
    return get_col_asset_spec_nb(c, c.col)
...
def get_col_order_value_nb(
    ...
    """
    exec_state = get_col_exec_state_nb(c, col, group, val_price=val_price)
    asset_spec = get_col_asset_spec_nb(c, col)
