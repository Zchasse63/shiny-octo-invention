...
def long_buy_nb(
    ...
    asset_spec: AssetSpec = NoAssetSpec,
    ...
    Args:
        ...
        asset_spec (AssetSpec): Asset specification.

            See `vectorbtpro.portfolio.enums.AssetSpec`.
        price_area (PriceArea): Price area constraint.
    ...
    if price >= 0:
        ...
        adj_price = price + abs(price) * slippage
    ...
    if np.isinf(size):
        ...
        order_value = size * adj_price * asset_spec.multiplier
        req_fees = abs(order_value) * fees + fixed_fees
        req_cash = order_value + req_fees

    abs_req_cash = abs(order_value) + req_fees if not np.isinf(size) else np.inf

    if is_close_or_less_nb(abs_req_cash, order_value_limit):
        final_size = size
        fees_paid = req_fees
    else:
        ...
        max_acq_size = max_req_cash / (abs(adj_price) * asset_spec.multiplier)
        ...
        if should_apply_size_granularity_nb(max_acq_size, size_granularity):
            ...
            new_order_value = final_size * adj_price * asset_spec.multiplier
            fees_paid = abs(new_order_value) * fees + fixed_fees
    ...
    if leverage_mode == LeverageMode.Lazy:
        ...
        if leverage > 1.0:
            order_value = final_size * adj_price * asset_spec.multiplier
            abs_order_value = abs(order_value)
            margin_cash = abs_order_value / leverage
            ...
            new_debt = _account_state.debt + abs_order_value * (leverage - 1.0) / leverage + extra_debt
        else:
            new_debt = _account_state.debt
            new_locked_cash = _account_state.locked_cash
...
def long_sell_nb(
    ...
    if price >= 0:
        ...
        adj_price = price - abs(price) * slippage
    adj_price = check_adj_price_nb(adj_price, price_area, is_closing_price, price_area_vio_mode)
    acq_cash = size_limit * adj_price * asset_spec.multiplier
    fees_paid = abs(acq_cash) * fees + fixed_fees
...
def short_sell_nb(
    ...
    fees_adj_price = abs(adj_price) * asset_spec.multiplier * ((1.0 / leverage) + fees)
    if fees_adj_price == 0:
    ...
    order_value = size_limit * adj_price * asset_spec.multiplier
    abs_order_value = abs(order_value)
    fees_paid = abs_order_value * fees + fixed_fees
    ...
    if np.isinf(leverage):
        ...
        if is_close_or_less_nb(_account_state.free_cash, fees_paid):
            return order_not_filled_nb(OrderStatus.Rejected, OrderStatusInfo.CantCoverFees), _account_state
        effective_leverage = abs_order_value / (_account_state.free_cash - fees_paid)
    else:
        effective_leverage = float(leverage)

    margin_cash = abs_order_value / effective_leverage
    ...
    new_debt = _account_state.debt + abs_order_value
    new_locked_cash = _account_state.locked_cash + margin_cash
...
def short_buy_nb(
    ...
    abs_req_cash = abs(order_value) + req_fees if not np.isinf(size_limit) else np.inf
...
def update_value_nb(
    ...
    if position_before != 0:
        asset_value_before = position_before * val_price_before * multiplier
    else:
        asset_value_before = 0.0
    if position_now != 0:
        asset_value_now = position_now * val_price_now * multiplier
    else:
        asset_value_now = 0.0
...
def approx_long_buy_value_nb(
    ...
    if size == 0:
        return 0.0
    order_value = abs(size) * val_price * multiplier
    add_free_cash = -order_value
    return -add_free_cash


@register_jitted(cache=True)
def approx_long_sell_value_nb(
    position: float,
    debt: float,
    val_price: float,
    size: float,
    ...
    order_value = size_limit * val_price * multiplier
...
@register_jitted(cache=True)
def approx_short_sell_value_nb(
    val_price: float,
    size: float,
    multiplier: float = 1.0,
) -> float:
    ...
    if size == 0:
        return 0.0
    order_value = abs(size) * abs(val_price) * multiplier
    add_free_cash = -order_value
    return -add_free_cash
...
def approx_buy_value_nb(
    ...
    if position <= 0 and direction == Direction.ShortOnly:
        return approx_short_buy_value_nb(position, debt, locked_cash, val_price, size, multiplier=multiplier)
    if position >= 0:
        return approx_long_buy_value_nb(val_price, size, multiplier=multiplier)
    value1 = approx_short_buy_value_nb(position, debt, locked_cash, val_price, size, multiplier=multiplier)
    new_size = add_nb(size, -abs(position))
    if new_size <= 0:
        return value1
    value2 = approx_long_buy_value_nb(val_price, new_size, multiplier=multiplier)
...
def approx_sell_value_nb(
    ...
    if position >= 0 and direction == Direction.LongOnly:
        return approx_long_sell_value_nb(position, debt, val_price, size, multiplier=multiplier)
    if position <= 0:
        return approx_short_sell_value_nb(val_price, size, multiplier=multiplier)
    value1 = approx_long_sell_value_nb(position, debt, val_price, size, multiplier=multiplier)
    new_size = add_nb(size, -abs(position))
    if new_size <= 0:
        return value1
    value2 = approx_short_sell_value_nb(val_price, new_size, multiplier=multiplier)
    return value1 + value2
...
def execute_order_nb(
    ...
    if not np.isfinite(asset_spec.multiplier) or asset_spec.multiplier <= 0:
        raise ValueError("asset_spec.multiplier must be finite and greater than 0")

    if np.isinf(price_area.open):
        raise ValueError("price_area.open must be either NaN or finite")
    if np.isinf(price_area.high):
        raise ValueError("price_area.high must be either NaN or finite")
    if np.isinf(price_area.low):
        raise ValueError("price_area.low must be either NaN or finite")
    if np.isinf(price_area.close):
        raise ValueError("price_area.close must be either NaN or finite")
    ...
    if np.isnan(order.size):
        return order_not_filled_nb(OrderStatus.Ignored, OrderStatusInfo.SizeNaN), re_exec_state
    if np.isnan(order_price):
        return order_not_filled_nb(OrderStatus.Ignored, OrderStatusInfo.PriceNaN), re_exec_state
    ...
    if (
        ...
        if np.isinf(val_price) or val_price == 0:
            raise ValueError("val_price_now must be finite and non-zero")
        if np.isnan(val_price):
            return order_not_filled_nb(OrderStatus.Ignored, OrderStatusInfo.ValPriceNaN), re_exec_state
        if (
            order_size_type == SizeType.ValuePercent100
            or order_size_type == SizeType.ValuePercent
            or order_size_type == SizeType.TargetPercent100
            ...
            if np.isnan(value):
                return order_not_filled_nb(OrderStatus.Ignored, OrderStatusInfo.ValueNaN), re_exec_state
            if value <= 0:
                return order_not_filled_nb(OrderStatus.Rejected, OrderStatusInfo.ValueZeroNeg), re_exec_state
    ...
    if not np.isnan(min_order_size):
        ...
        if not np.isnan(percent) and not np.isnan(min_percent) and is_less_nb(percent, min_percent):
            return order_not_filled_nb(OrderStatus.Ignored, OrderStatusInfo.MinSizeNotReached), re_exec_state
    ...
    if order.reject_prob > 0:
        if np.random.uniform(0, 1) < order.reject_prob:
            return order_not_filled_nb(OrderStatus.Rejected, OrderStatusInfo.RandomEvent), re_exec_state
...
def fill_log_record_nb(
    ...
    records["idx"][r, col] = i
    records["asset_spec_multiplier"][r, col] = asset_spec.multiplier
...
def process_order_nb(
    ...
    if log_records is not None and log_counts is not None:
        if order.log and log_records.shape[0] > 0:
            ...
            order_id = order_counts[col] - 1 if order_counts is not None and is_filled else -1
            fill_log_record_nb(
                records=log_records,
                r=log_counts[col],
                group=group,
...
def prepare_last_cash_nb(
    ...
    if cash_sharing:
        ...
        for group in range(len(group_lens)):
            last_cash[group] = flex_select_1d_pc_nb(init_cash, group)
    else:
        last_cash = np.empty(target_shape[1], dtype=float_)
        for col in range(target_shape[1]):
            last_cash[col] = flex_select_1d_pc_nb(init_cash, col)
    return last_cash
...
def prepare_last_position_nb(target_shape: tp.Shape, init_position: tp.FlexArray1d) -> tp.Array1d:
    ...
    for col in range(target_shape[1]):
        last_position[col] = flex_select_1d_pc_nb(init_position, col)
...
def prepare_last_value_nb(
    ...
    Args:
        ...
        multiplier (FlexArray1d): Array of contract multipliers.
    ...
    if cash_sharing:
        ...
        for group in range(len(group_lens)):
            ...
            _init_cash = flex_select_1d_pc_nb(init_cash, group)
            last_value[group] = _init_cash
            for col in range(from_col, to_col):
                _init_position = flex_select_1d_pc_nb(init_position, col)
                _init_price = flex_select_1d_pc_nb(init_price, col)
                _multiplier = flex_select_1d_pc_nb(multiplier, col)
                if _init_position != 0:
                    last_value[group] += _init_position * _init_price * _multiplier
            from_col = to_col
    else:
        ...
        for col in range(target_shape[1]):
            ...
            if _init_position == 0:
                ...
                last_value[col] = _init_cash + _init_position * _init_price * _multiplier
    return last_value
...
def prepare_last_pos_info_nb(
    ...
    if fill_pos_info:
        ...
        for col in range(target_shape[1]):
            ...
            _multiplier = flex_select_1d_pc_nb(multiplier, col)
            if _init_position != 0:
                fill_init_pos_info_nb(last_pos_info[col], col, _init_position, _init_price, multiplier=_multiplier)
    else:
...
def get_trade_stats_nb(
    ...
    direction: int,
    multiplier: float = 1.0,
    use_entry_size_sign: bool = False,
) -> tp.Tuple[float, float]:
    ...
    Args:
        ...
        multiplier (float): Contract multiplier.
        use_entry_size_sign (bool): Whether to normalize return by signed entry notional.

    Returns:
        Tuple[float, float]: Tuple containing the profit and loss (pnl) and the return ratio.
    """
    entry_val = size * entry_price * multiplier
    exit_val = size * exit_price * multiplier
    val_diff = add_nb(exit_val, -entry_val)
    if val_diff != 0 and direction == TradeDirection.Short:
        val_diff *= -1
    pnl = val_diff - entry_fees - exit_fees
    if use_entry_size_sign:
        ret_entry_val = size * entry_price * multiplier
    else:
        ret_entry_val = abs(entry_val)
    if is_close_nb(ret_entry_val, 0):
        ret = np.nan
    else:
        ret = pnl / ret_entry_val
    return pnl, ret


@register_jitted(cache=True)
def update_open_pos_info_stats_nb(
    record: tp.Record,
    position_now: float,
    price: float,
    multiplier: float = 1.0,
...
def fill_init_pos_info_nb(
    ...
    update_open_pos_info_stats_nb(record, position_now, np.nan, multiplier=multiplier)
...
def update_pos_info_nb(
    ...
    if order_result.status == OrderStatus.Filled:
        ...
        update_open_pos_info_stats_nb(record, position_now, order_result.price, multiplier=multiplier)
        ...
        if hit_below:
            if np.isinf(limit_delta) and delta_format != DeltaFormat.Target:
                ...
                if delta_format == DeltaFormat.Absolute:
                    ...
                    limit_price = init_price - abs(init_price) * limit_delta
...
def resolve_stop_price_nb(
    ...
    if hit_below:
        if delta_format == DeltaFormat.Absolute:
            ...
            stop_price = init_price - abs(init_price) * abs(stop)
        elif delta_format == DeltaFormat.Target:
            stop_price = stop
        else:
            raise ValueError("Invalid DeltaFormat option")
