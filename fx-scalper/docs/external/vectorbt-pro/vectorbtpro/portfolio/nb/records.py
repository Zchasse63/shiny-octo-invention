    ...
    for col in prange(col_lens.shape[0]):
                ...
                if last_position_id != -1:
                    if trade_r - from_trade_r > 1:
                        fill_position_record_nb(
                            new_records[:, col],
                            counts[col],
                            trade_records[from_trade_r:trade_r],
                            multiplier=_multiplier,
                        )
                    else:
        ...
        if trade_r - from_trade_r > 0:
            fill_position_record_nb(
                new_records[:, col],
                counts[col],
                trade_records[from_trade_r : trade_r + 1],
                multiplier=_multiplier,
            )
        else:
            copy_trade_record_nb(new_records[:, col], counts[col], trade_records[from_trade_r])
...
def price_status_nb(
    ...
    for i in range(len(order_records)):
        ...
        if high is not None:
            _high = flex_select_nb(high, order["idx"], order["col"])
        else:
            _high = np.nan
        if low is not None:
...
def trade_mfe_nb(
    ...
    if direction == TradeDirection.Long:
        if use_returns:
            return (best_price - entry_price) / abs(entry_price)
        return (best_price - entry_price) * size
    if use_returns:
        return (entry_price - best_price) / abs(best_price)
...
def trade_mae_nb(
    ...
    if direction == TradeDirection.Long:
        if use_returns:
            return (worst_price - entry_price) / abs(entry_price)
        return (worst_price - entry_price) * size
    if use_returns:
        return (entry_price - worst_price) / abs(worst_price)
