...
def position_mask_nb(
    ...
    """
    out = np.full(assets.shape, False, dtype=bool_)
...
def position_mask_grouped_nb(
    ...
    """
    out = np.full((assets.shape[0], len(group_lens)), False, dtype=bool_)
...
def get_free_cash_diff_nb(
    ...
    final_cash = -size * price * multiplier - fees
    if is_close_nb(size, 0):
        ...
        if position_now < 0:
            if position_before < 0:
                short_size = abs(size)
                free_cash_diff = -abs(short_size * price * multiplier) - fees
            else:
                short_size = abs(position_now)
                free_cash_diff = position_before * price * multiplier - abs(short_size * price * multiplier) - fees
            short_value = abs(short_size * price * multiplier)
...
def cash_flow_nb(
    ...
    order_records: tp.RecordArray,
    col_map: tp.GroupMap,
    multiplier: tp.FlexArray1dLike = 1.0,
    ...
    Args:
        ...
        order_records (RecordArray): Array of order records.
            ...
            Must adhere to the `vectorbtpro.portfolio.enums.order_dt` dtype.
        col_map (GroupMap): Tuple of indices and lengths for each column.
        multiplier (FlexArray1dLike): Contract multiplier.
    ...
    for col in prange(col_lens.shape[0]):
        ...
        _multiplier = flex_select_1d_pc_nb(multiplier_, col)
        ...
        for c in range(col_len):
            ...
            if free:
                ...
                cash_flow = -size * price * _multiplier - fees
...
def init_position_value_nb(
    ...
    for col in range(n_cols):
        _init_position = flex_select_1d_pc_nb(init_position_, col)
        _init_price = flex_select_1d_pc_nb(init_price_, col)
        _multiplier = flex_select_1d_pc_nb(multiplier_, col)

        if _init_position == 0:
            out[col] = 0.0
        else:
            out[col] = _init_position * _init_price * _multiplier
    return out
...
def init_position_value_grouped_nb(
    ...
    for group in prange(len(group_lens)):
        ...
        for col in range(from_col, to_col):
            ...
            if _init_position != 0:
                out[group] += _init_position * _init_price * _multiplier
    return out
    ...
    for col in prange(close.shape[1]):
        ...
        for i in range(_sim_start, _sim_end):
            if assets[i, col] == 0:
                ...
                out[i, col] = close[i, col] * assets[i, col] * _multiplier
    return out
...
def total_profit_nb(
    ...
    for col in prange(target_shape[1]):
        ...
        if _init_position != 0:
            ...
            cash[col] = -_init_position * _init_price * _multiplier
        _sim_start = sim_start_[col]
        _sim_end = sim_end_[col]
        if _sim_start >= _sim_end:
            continue
    ...
    for col in prange(col_lens.shape[0]):
        ...
        if col_len != 0:
            ...
            for c in range(col_len):
                ...
                if order_record["side"] == OrderSide.Buy:
                    order_cash = order_record["size"] * order_record["price"] * _multiplier + order_record["fees"]
        ...
        if assets[col] == 0 and cash[col] == 0:
            ...
            total_profit[col] = cash[col] + assets[col] * close[_sim_end - 1, col] * _multiplier
    return total_profit
