...
Fields:
    ...
    Value: Specifies the monetary value of assets to trade. 
        ...
        It is converted to `SizeType.Amount` using `ExecState.val_price` and `AssetSpec.multiplier`.
    ...
    TargetValue: Specifies the target asset value using `ExecState.val_price` and `AssetSpec.multiplier`.
...
class AssetSpec(tp.NamedTuple):
    multiplier: float


__pdoc__["AssetSpec"] = "Asset specification."

NoAssetSpec = AssetSpec(multiplier=1.0)
"""_"""

__pdoc__["NoAssetSpec"] = "`AssetSpec` instance with default values, representing the absence of asset specifications."
...
"""
__pdoc__[
    "SimulationContext.multiplier"
] = """Contract multiplier per column.
...
log_fields = [
    ...
    ("req_allow_partial", bool_),
    ("req_raise_reject", bool_),
    ("req_log", bool_),
    ("res_size", float_),
    ("res_price", float_),
    ("res_fees", float_),
...
Fields:
    ...
    col: Column index.
    idx: Row index.
    asset_spec_multiplier: Contract multiplier.
