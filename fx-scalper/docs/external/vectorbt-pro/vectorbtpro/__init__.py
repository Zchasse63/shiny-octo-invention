...
def load_dotenv(dotenv_path: typing.Optional[typing.Union[str, Path]] = None, override: bool = False) -> bool:
    ...
    - blank lines
    - export prefixes such as `export KEY=VALUE`

    Existing environment variables are preserved unless `override` is True.

    Args:
        dotenv_path (Optional[Union[str, Path]]): Path to the `.env` file.

            If not provided, uses `Path.cwd() / ".env"`.
        override (bool): Whether to overwrite keys that already exist in `os.environ`.

    Returns:
        bool: True if the file existed and was processed, otherwise False.
    """
    path = Path(dotenv_path).expanduser() if dotenv_path is not None else Path.cwd() / ".env"
    if not path.is_file():
        return False
...
dotenv_path = os.environ.get("VBT_DOTENV_PATH", None)
...
if typing.TYPE_CHECKING:
    ...
    from vectorbtpro.cli import *
    from vectorbtpro.mcp_server import *
    from vectorbtpro.mcp import *
...
__blacklist__ = ["__main__", "cli", "mcp_server"]
