...
def suggest_compression(file_name: str) -> tp.Optional[str]:
    ...
    """
    suffixes = [s[1:].lower() for s in Path(file_name).suffixes]
...
def suggest_file_path(
    path: tp.PathLike,
    compression: tp.CompressionLike = None,
    append_suffix: tp.Optional[bool] = None,
) -> Path:
    """Suggest a file path with appropriate serialization/compression extensions.

    Args:
        path (PathLike): Original file path.
        compression (CompressionLike): Compression algorithm.

            See `compress`.
        append_suffix (Optional[bool]): Whether to append serialization/compression extensions.

            If None, uses the default from `vectorbtpro._settings.pickling`.

    Returns:
        Path: Suggested file path with an appropriate extension.
    """
    from vectorbtpro._settings import settings
    ...
    if append_suffix is None:
        append_suffix = pickling_cfg["append_suffix"]
    if append_suffix:
        suffixes = [s[1:].lower() for s in path.suffixes]
        has_serialization = any(s in get_serialization_extensions("pickle") for s in suffixes)
        has_compression = any(s in get_compression_extensions() for s in suffixes)
        if not has_serialization and not has_compression:
            path = path.with_suffix(path.suffix + ".pickle")
        if compression not in (None, False) and not has_compression:
            path = path.with_suffix(path.suffix + "." + compression)
    return path


def extensions_match(a: str, b: str, ext_type: str) -> bool:
    """Check if two extensions belong to the same family.

    For example, "pkl" and "pickle" both belong to the "pickle" serialization family,
    and "gz" and "gzip" both belong to the "gzip" compression family.

    Args:
        a (str): First extension.
        b (str): Second extension.
        ext_type (str): Extension type ("serialization" or "compression").

    Returns:
        bool: True if both extensions are in the same group, False otherwise.
    """
    from vectorbtpro._settings import settings
...
def resolve_file_path(
    ...
    """Resolve a file path for loading by searching for matching files.

    The parent directory is searched for files matching the base name with recognized
    serialization/compression extensions that align with the pickle format and `compression` argument.

    Args:
        path (PathLike): File path to resolve.
        compression (CompressionLike): Compression algorithm.

            See `compress`.
        raise_error (bool): Whether to raise an error if no matching file is found.

    Returns:
        Optional[Path]: Resolved file path, or None if not found and `raise_error` is False.
    ...
    if isinstance(compression, bool) and compression:
        ...
        compression = settings["pickling"]["compression"]
        if compression is None:
            raise ValueError("Set default compression in settings")

    pickle_exts = get_serialization_extensions("pickle")
    compression_exts = get_compression_extensions()

    def _strip_recognized_suffixes(p):
        bare = p
        ser_ext = None
        comp_ext = None
        if bare.suffix and bare.suffix[1:].lower() in compression_exts:
            comp_ext = bare.suffix[1:].lower()
            bare = bare.with_suffix("")
        if bare.suffix and bare.suffix[1:].lower() in pickle_exts:
            ser_ext = bare.suffix[1:].lower()
            bare = bare.with_suffix("")
        return bare, ser_ext, comp_ext

    def _matches_candidate(candidate, req_bare, req_ser, req_comp):
        cand_bare, cand_ser, cand_comp = _strip_recognized_suffixes(candidate)
        if cand_bare != req_bare:
            return False
        if cand_ser is None and cand_comp is None:
            return False
        if req_ser is not None:
            if cand_ser is None or not extensions_match(cand_ser, req_ser, "serialization"):
                return False
        if req_comp is False:
            if cand_comp is not None:
                return False
        elif req_comp is not None:
            if cand_comp is None or not extensions_match(cand_comp, req_comp, "compression"):
                return False
        return True

    def _check_exact_path(p, req_ser, req_comp):
        if not p.is_file():
            return False, None, None
        _, exact_ser, exact_comp = _strip_recognized_suffixes(p)
        if req_ser is not None and exact_ser is not None:
            if not extensions_match(exact_ser, req_ser, "serialization"):
                return False, exact_ser, exact_comp
        if req_comp is False and exact_comp is not None:
            return False, exact_ser, exact_comp
        if req_comp not in (None, False) and exact_comp is not None:
            if not extensions_match(exact_comp, req_comp, "compression"):
                return False, exact_ser, exact_comp
        return True, exact_ser, exact_comp

    req_bare, req_ser, req_path_comp = _strip_recognized_suffixes(path)
    if compression is None:
        req_comp = req_path_comp
    elif compression is False:
        req_comp = False
    else:
        req_comp = compression

    exact_matches, exact_ser, exact_comp = _check_exact_path(path, req_ser, req_comp)
    if exact_matches:
        more_specific = False
        if req_ser is not None and exact_ser is None:
            more_specific = True
        if req_comp not in (None, False) and exact_comp is None:
            more_specific = True
        if not more_specific:
            return path

    parent = req_bare.parent
    if not parent.is_dir():
        if not raise_error:
            return None
        raise FileNotFoundError(f"Directory does not exist: {str(parent)!r}")
    ...
    for p in parent.iterdir():
        ...
        if p == path:
            continue
        if _matches_candidate(p, req_bare, req_ser, req_comp):
            candidates.append(p)
    ...
    if len(candidates) > 1:
        if not raise_error:
            return None
        raise ValueError(
            f"Multiple files found matching {str(path)!r} "
            f"with serialization={req_ser!r} and compression={req_comp!r}: "
            f"{candidates}. Please provide a more specific path or extension."
        )
    ...
    raise FileNotFoundError(
        f"No file found matching {str(path)!r} with serialization={req_ser!r} and compression={req_comp!r}"
    )
...
def save_bytes(
    ...
    """
    path = suggest_file_path(path, compression=compression, append_suffix=append_suffix)
    file_name = None
    if compression is None:
...
def load_bytes(
    ...
    """
    path = resolve_file_path(path, compression=compression)
            ...
            if p.is_file():
                if p.stem.split(".")[0] == path.stem.split(".")[0]:
                    ...
                    if file_format is None:
                        ...
                        if not extensions_match(suffixes[0], file_format, "serialization"):
                            continue
                    if compression is False:
                        ...
                        if len(suffixes) == 1 or not extensions_match(suffixes[1], compression, "compression"):
                            continue
                    paths.append(p)
        if len(paths) == 1:
            return paths[0]
