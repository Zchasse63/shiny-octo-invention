...
from vectorbtpro.utils.pickling import load, save, resolve_file_path
    ...
    * `post_execute_func`: Called after processing all tasks. Should return either None to retain default
    ...
    def call_pre_chunk_func(
        ...
        if cache_chunks:
            ...
            chunk_path = chunk_cache_dir / ("chunk_%d" % chunk_idx)
            if chunk_cache_load_kwargs is None:
                chunk_cache_load_kwargs = {}
            compression = chunk_cache_load_kwargs.get("compression", None)
            chunk_path = resolve_file_path(chunk_path, compression=compression, raise_error=False)
    ...
    def call_post_execute_func(
        ...
        if cache_chunks and release_chunk_cache:
            ...
            if chunk_cache_load_kwargs is None:
                chunk_cache_load_kwargs = {}
            chunk_paths = chunk_cache_dir.rglob("chunk_*")
