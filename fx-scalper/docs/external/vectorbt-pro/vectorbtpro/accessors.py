...
def collect_accessor_names(cls: tp.Type[DirNamesMixin]) -> tp.Set[str]:
    """Collect registered accessor names without resolving their descriptors.

    Args:
        cls (Type[DirNamesMixin]): Accessor class to inspect.

    Returns:
        Set[str]: Names of accessor descriptors defined across the class hierarchy.
    """
    accessor_names = set()
    for base_cls in cls.__mro__:
        for name, attr in base_cls.__dict__.items():
            if isinstance(attr, (Accessor, CachedAccessor)):
                accessor_names.add(name)
    return accessor_names


def ensure_own_accessors(cls: tp.Type[DirNamesMixin]) -> None:
    """Ensure that an accessor class owns its accessor registry.

    Args:
        cls (Type[DirNamesMixin]): Accessor class to update.

    Returns:
        None
    """
    if "_accessors" not in cls.__dict__:
        cls._accessors = set(cls._accessors)
...
class Vbt_IDXAccessor(DirNamesMixin, BaseIDXAccessor):
    ...
    def _dir_additions(self) -> tp.Set[str]:
        return collect_accessor_names(type(self))
